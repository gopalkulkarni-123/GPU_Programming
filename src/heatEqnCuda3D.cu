        ...
        for (int i = 0; i < blockBreadth; ++i) {
            for (int j = 0; j < blockLength; ++j) { 
                for (int k = 0; k < blockDepth; ++k) {
                    ...
                    localGrid[i * width + j] = dGrid[(xMin + i) * (N*N) + (yMin + j) * N + (zMin + k)] + tempDiff; // index = k * (height * width) + j * width + i;
...
int main() {
    ...
    cudaMalloc(&deviceLocalGrids, sizeof(float) * NUM_BLOCKS * BLOCK_SIZE * BLOCK_SIZE); // Increase the size to accomodate the z direction
    cudaMalloc(&deviceMainGrid, sizeof(float) * N * N); // Increase the size to accomodate the z direction
